...
dbutils.fs.rm("checkpointdir2",True)

# COMMAND ----------

dbutils.fs.rm("dbfs:/user/hive/warehouse/orders_result_final1",True)
...
def myfunction(orders_result, batch_id):
    ...
    merge_statement = """merge into orders_result_final1 t using orders_result s 
...
spark.sql("select * from orders_result_final1").show()
...
spark.sql("create table orders_result_final1(customer_id long,orders_placed long,products_purchased long,amount_spent float)")
